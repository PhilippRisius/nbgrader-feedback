...
.. warning::
    ...
    It is important to be aware that any changes to files found within the ``src/nbgrader_feedback`` folder (with the exception of ``src/nbgrader_feedback/__init__.py``) will trigger the ``bump-version.yml`` workflow. Be careful not to commit changes to files in this folder when preparing a new release.
...
The new version based off of the version checked out will now be available via `pip` (`pip install nbgrader-feedback`).
...
In order to create a new conda build recipe, to be used when proposing packages to the conda-forge repository, we strongly suggest using the `grayskull` tool:
   ...
   .. code-block:: console
        ...
        grayskull pypi nbgrader-feedback
 ...
 * If possible, configure tests within the conda-forge build CI (e.g. `imports: nbgrader_feedback`, `commands: pytest nbgrader-feedback`).
...
From the nbgrader-feedback source folder we can enter into the docker container, providing access to the `src/nbgrader_feedback` source files by linking them to the running image:

    .. code-block:: console

        sudo docker run --rm -ti -v $(pwd):/src/nbgrader_feedback -w /src/nbgrader_feedback quay.io/pypa/manylinux_2_24_x86_64 bash
...
This will then place two files in `nbgrader-feedback/dist/` ("nbgrader_feedback-1.2.3-py3-none-any.whl" and "nbgrader-feedback-1.2.3.tar.gz").
